...
Stores every submitted workflow and its processing state.

| Column | Type | Purpose |
| --- | --- | --- |
| `id` | `UUID PRIMARY KEY` | Generated in `POST /api/workflows` using `crypto.randomUUID()`. |
| `title` | `TEXT NOT NULL` | User-provided title; also used as the Kafka message key. |
| `text` | `TEXT NOT NULL` | User-provided technical content to summarize/classify. |
| `status` | `VARCHAR(20)` | One of `pending`, `queued`, `processing`, `completed`, `failed`, or `dlq` in TypeScript; actual writes use `queued`, `processing`, `completed`, and `dlq`. |
| `current_stage` | `VARCHAR(30)` | One of `ingestion`, `summarization`, `classification`, `audit`. |
| `summary` | `TEXT` | Gemini or fallback summary. |
| `sentiment` | `VARCHAR(10)` | Gemini or fallback sentiment. |
| `entities` | `TEXT[]` | Extracted technologies/concepts, indexed with GIN. |
| `error` | `TEXT` | Pipeline error when status is `dlq`. |
| `created_at` | `TIMESTAMPTZ` | Original workflow timestamp. |

Indexes:

| Index | Type | Purpose |
| --- | --- | --- |
| `idx_workflows_entities_gin` | `GIN(entities)` | Fast array/tag lookup using operators such as `@>`. |
| `idx_workflows_status` | `B-tree(status)` | Fast status filtering. |
| `idx_workflows_created_at` | `B-tree(created_at DESC)` | Fast dashboard ordering by newest workflows. |

### `system_logs`

Stores human-readable events produced by the gateway, Kafka layer, Redis rate limiter, database layer, and workers.

| Column | Type | Purpose |
| --- | --- | --- |
| `id` | `UUID PRIMARY KEY DEFAULT gen_random_uuid()` | Unique log ID generated by PostgreSQL. |
| `level` | `VARCHAR(10)` | `info`, `warn`, `error`, or `success`. |
| `component` | `VARCHAR(20)` | `GATEWAY`, `KAFKA`, `REDIS`, `DATABASE`, or `WORKER`. |
| `message` | `TEXT` | Displayed in the dashboard telemetry log. |
...
| Index | Type | Purpose |
| --- | --- | --- |
| `idx_system_logs_created_at` | `B-tree(created_at DESC)` | Fast latest-log dashboard reads. |

### `database_query_logs`

Stores query audit data shown in the PostgreSQL trace panel.

| Column | Type | Purpose |
| --- | --- | --- |
| `id` | `UUID PRIMARY KEY DEFAULT gen_random_uuid()` | Unique query log ID generated by PostgreSQL. |
| `query` | `TEXT` | Human-readable SQL or SQL-like operation text. |
| `execution_time_ms` | `DECIMAL(10,3)` | Measured or supplied execution time. |
| `index_used` | `BOOLEAN` | Used by the UI to display index scan vs sequential scan. |
| `created_at` | `TIMESTAMPTZ` | Query log time. |
